...
past_tourney_history <- function(df) 
{
  ...
  team_games <- team1 %>% 
    ...
    arrange(Team, Season) %>%
    group_by(Team) %>% 
    mutate(num_tourney_games_since_1998 = cumsum(num_games),
           num_tourney_advanced_since_1998 = cumsum(advanced),
           num_tourney_sweet_16_since_1998 = cumsum(sweet_16),
           num_tourney_elite_8_since_1998 = cumsum(elite_8),
           num_tourney_final_4_since_1998 = cumsum(final_4)) %>%
    ungroup() %>% arrange(Team, Season) %>% group_by(Team) %>%
    mutate(num_tourney_games_since_1998 = dplyr::lag(num_tourney_games_since_1998, n = 1, default = 0),
           num_tourney_advanced_since_1998 = dplyr::lag(num_tourney_advanced_since_1998, n = 1, default = 0),
           num_tourney_sweet_16_since_1998 = dplyr::lag(num_tourney_sweet_16_since_1998, n = 1, default = 0),
           num_tourney_elite_8_since_1998 = dplyr::lag(num_tourney_elite_8_since_1998, n = 1, default = 0),
           num_tourney_final_4_since_1998 = dplyr::lag(num_tourney_final_4_since_1998, n = 1, default = 0)) %>% 
    dplyr::select(Season, Team, num_tourney_games_since_1998, num_tourney_advanced_since_1998,
                  num_tourney_sweet_16_since_1998, num_tourney_elite_8_since_1998,
                  num_tourney_final_4_since_1998)
  ...
  team_games_recent <- team1 %>% 
    ...
    arrange(Team, Season) %>%
    group_by(Team) %>% 
    summarise(num_tourney_games_since_1998 = sum(num_games),
              num_tourney_advanced_since_1998 = sum(advanced),
              num_tourney_sweet_16_since_1998 = sum(sweet_16),
              num_tourney_elite_8_since_1998 = sum(elite_8),
              num_tourney_final_4_since_1998 = sum(final_4),
              .groups = "drop") %>%
    mutate(Season = 2024) %>% 
    dplyr::select(Season, Team, num_tourney_games_since_1998, num_tourney_advanced_since_1998,
                  num_tourney_sweet_16_since_1998, num_tourney_elite_8_since_1998,
...
get_coaches_reg_season_record <- function(df)
{
  ...
  winners <- df %>% #filter(W_CoachName == 'kevin_nickelberry') %>% 
    ...
    mutate(Career_Num_Wins_Since_1998 = cumsum(WTeamID_Won),
           Career_Num_Wins_Since_1998 = dplyr::lag(Career_Num_Wins_Since_1998, n = 1, default = 0)) %>%
    rename('Team' = WTeamID, 'CoachName' = 'W_CoachName') %>% 
    dplyr::select(Season, DayNum, Team, CoachName, Career_Num_Wins_Since_1998)
  
  losers <- df %>% #filter(L_CoachName == 'kevin_nickelberry') %>% 
    arrange(Season, DayNum) %>%
    group_by(L_CoachName) %>% 
    mutate(Career_Num_Losses_Since_1998 = cumsum(LTeamID_Lost),
           Career_Num_Losses_Since_1998 = dplyr::lag(Career_Num_Losses_Since_1998, n = 1, default = 0)) %>%
    rename('Team' = LTeamID, 'CoachName' = 'L_CoachName') %>% 
    dplyr::select(Season, DayNum, Team, CoachName, Career_Num_Losses_Since_1998)
  
  merged <- dplyr::bind_rows(winners, losers) %>% 
    arrange(CoachName, Season, DayNum) %>%
    group_by(CoachName) %>% 
    fill(Career_Num_Wins_Since_1998, .direction = "up") %>%
    fill(Career_Num_Wins_Since_1998) %>%
    fill(Career_Num_Losses_Since_1998, .direction = "up") %>%
    fill(Career_Num_Losses_Since_1998) %>%
    tidyr::replace_na(list(Career_Num_Wins_Since_1998 = 0, Career_Num_Losses_Since_1998 = 0)) %>% 
    mutate(Career_Win_Pct_Since_1998 = Career_Num_Wins_Since_1998 / (Career_Num_Wins_Since_1998 + Career_Num_Losses_Since_1998)) %>% 
    tidyr::replace_na(list(Career_Win_Pct_Since_1998 = 0.5)) %>% 
...
get_coaches_tourney_record <- function(df)
{
  ...
  winners <- df %>% #filter(WTeamID == 1181) %>% 
    arrange(Season, DayNum) %>%
    group_by(W_CoachName) %>% 
    mutate(Career_Tourney_Wins_Since_1998 = cumsum(WTeamID_Won),
           Career_Tourney_Wins_Since_1998 = dplyr::lag(Career_Tourney_Wins_Since_1998, n = 1, default = 0)) %>%
    rename('Team' = WTeamID, 'CoachName' = 'W_CoachName') %>% 
    dplyr::select(Season, DayNum, Team, CoachName, Career_Tourney_Wins_Since_1998)
  
  losers <- df %>% #filter(LTeamID == 1181) %>% 
    arrange(Season, DayNum) %>%
    group_by(L_CoachName) %>% 
    mutate(Career_Tourney_Losses_Since_1998 = cumsum(LTeamID_Lost),
           Career_Tourney_Losses_Since_1998 = dplyr::lag(Career_Tourney_Losses_Since_1998, n = 1, default = 0)) %>%
    rename('Team' = LTeamID, 'CoachName' = 'L_CoachName') %>% 
    dplyr::select(Season, DayNum, Team, CoachName, Career_Tourney_Losses_Since_1998)
  
  merged <- dplyr::bind_rows(winners, losers) %>% 
    arrange(CoachName, Season, DayNum) %>%
    group_by(CoachName) %>% 
    fill(Career_Tourney_Wins_Since_1998, .direction = "up") %>%
    fill(Career_Tourney_Wins_Since_1998) %>%
    fill(Career_Tourney_Losses_Since_1998, .direction = "up") %>%
    fill(Career_Tourney_Losses_Since_1998) %>%
    tidyr::replace_na(list(Career_Tourney_Wins_Since_1998 = 0, Career_Tourney_Losses_Since_1998 = 0)) %>% 
    mutate(Career_Tourney_Pct_Since_1998 = Career_Tourney_Wins_Since_1998 / (Career_Tourney_Wins_Since_1998 + Career_Tourney_Losses_Since_1998)) %>% 
    tidyr::replace_na(list(Career_Tourney_Pct_Since_1998 = 0.5)) %>% 
    group_by(Season, Team, CoachName) %>% 
    filter(DayNum == min(DayNum)) %>% select(-DayNum)
  
  winners <- df %>% #filter(WTeamID == 1181) %>%
    group_by(W_CoachName) %>% 
    summarise(Career_Tourney_Wins_Since_1998 = sum(WTeamID_Won), .groups = "drop") %>%
    rename('CoachName' = 'W_CoachName') %>% 
    mutate(Season = 2024)
  
  losers <- df %>% #filter(LTeamID == 1181) %>% 
    group_by(L_CoachName) %>% 
    summarise(Career_Tourney_Losses_Since_1998 = sum(LTeamID_Lost), .groups = "drop") %>%
...
get_team_diffs <- function(df)
{
  ...
  stats <- c("Avg_Score","Avg_FGM","Avg_FGA","Avg_FGM3","Avg_FGA3","Avg_FTM","Avg_FTA",
             ...
             "Avg_Pace","Avg_3PRate","Avg_FTRate","Avg_Stl_Pct","SOS",
             "Double_Digit_Seed","Power")
...
run_glmnet <- function(vars, parametersGrid, k_fold)
{
  ...
  glmnet <- train(as.formula(paste0("as.factor(Team1_Victory) ~ ", paste0(vars, collapse = " + "))), 
                  data = train_continuous, method = "glmnet", tuneGrid=parametersGrid, 
...
run_model <- function(vars, model_type, tuneLength, k_fold, plot = FALSE)
{
  ...
  preds_prob <- ifelse(preds_prob > 0.94, 0.99999, preds_prob)
  preds_prob <- ifelse(preds_prob < 0.06, 0.00001, preds_prob)
...
# SummaryFunction = BigSummary, metric = "Brier", maximize = FALSE
